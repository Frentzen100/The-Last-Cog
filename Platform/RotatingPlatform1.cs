...
public class RotatingPlatform1 : MonoBehaviour
{
    ...
    public Image progressFill; // Reference to the progress bar UI
    ...
    void UpdateProgressBar()
{
    if (progressFill != null)
    {
        ...
        // Reverse the fill: 1 when at angleA, 0 when at angleB
        float progress = 1f - ((currentAngle - angleA) / (angleB - angleA));

        // Ensure values stay between 0 and 1
        progressFill.fillAmount = Mathf.Clamp(progress, 0f, 1f);

        Debug.Log("Progress Bar Fill: " + progressFill.fillAmount);
    }
}
